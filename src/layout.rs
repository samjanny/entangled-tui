...
pub fn lay_out(scene: &Scene, width: usize) -> Vec<String> {
    ...
    for line in logical {
        match line {
            LogicalLine::Wrap(s) => wrap_into(&s, width, &mut out),
...
fn node_lines(node: &SceneNode, out: &mut Vec<LogicalLine>) {
    ...
    let blank = || LogicalLine::Wrap(String::new());
    match node {
        SceneNode::Paragraph { runs } => {
            out.push(LogicalLine::Wrap(runs_to_string(runs)));
            out.push(blank());
        }
        SceneNode::Heading { level, runs } => {
            let hashes = "#".repeat(*level as usize);
            out.push(LogicalLine::Wrap(format!(
                "{hashes} {}",
                runs_to_string(runs)
            )));
            out.push(blank());
        ...
        SceneNode::Quote { runs, attribution } => {
            out.push(LogicalLine::Wrap(format!("> {}", runs_to_string(runs))));
            if let Some(attr) = attribution {
                out.push(LogicalLine::Wrap(format!("> -- {}", runs_to_string(attr))));
            }
            out.push(blank());
        }
        ...
        SceneNode::Divider => {
            out.push(LogicalLine::Wrap("---".to_owned()));
            out.push(blank());
        }
        SceneNode::Image { image } => {
            if image.alt.is_empty() {
                out.push(LogicalLine::Wrap("[image]".to_owned()));
            } else {
                out.push(LogicalLine::Wrap(format!(
                    "[image: {}]",
                    escape(&image.alt)
            ...
            if let Some(caption) = &image.caption {
                out.push(LogicalLine::Wrap(format!("({})", escape(caption))));
            }
            out.push(blank());
        }
        ...
        SceneNode::SubmitForm {
            ...
            for field in fields {
                out.push(LogicalLine::Wrap(format!("  {}", form_field(field))));
            }
            out.push(LogicalLine::Wrap(format!("[{}]", escape(submit_label))));
            out.push(blank());
        }
        ...
        SceneNode::Note {
            ...
            out.push(LogicalLine::Wrap(head));
            out.push(LogicalLine::Wrap(runs_to_string(runs)));
            out.push(blank());
        }
    }
...
/// A blank input line produces one blank output row. Wrapping is by Unicode
/// scalar count (a deliberate approximation; grapheme/display-width handling is
/// a later refinement).
fn wrap_into(line: &str, width: usize, out: &mut Vec<String>) {
    ...
    for word in line.split(' ') {
        ...
        if current_len == 0 {
            ...
            push_word(word, width, &mut current, &mut current_len, out);
